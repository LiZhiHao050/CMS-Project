package com.lizhihao.cms.comons;
...
public final class PageUtils {
	 ...
	 */
	  public static String pageLoad(int page, int pages, String url, int pageSize) {
          ...
          sb.append(String.format("<ul class='pagination'><li class='page-item'><a class='page-link'  href='javascript:void(0)' data='%1$s%2$s%3$s=%4$s' aria-label='Previous'>  <span aria-hidden='true'>&laquo;</span></a></li>" ,url, flag,"pageNum" ,page- 1 < 1 ? 1 : page- 1));
          //设开始页为1
          int beginPage = 1;
          //中间页为 pageSize/2
          ...
          sb.append(String.format("<li class='page-item'><a  class='page-link' href='javascript:void(0)' data='%1$s%2$s%3$s=%4$s' aria-label='Next' ><span aria-hidden='true'>&raquo;</span></a></li>" ,url, flag,"pageNum" ,page+ 1 >= pages ? pages : page+ 1));
          sb.append("</ul></nav>");
          return sb.toString();
      }
	...
	public static void page(HttpServletRequest request, String url, Integer pageSize, List<?> list, Long listCount, Integer page) {
		...
		if(page > 1) {
			prev = "<a href='"+request.getContextPath()+url+flag+"pageNum="+(page-1)+"'>上一页</a>";
		} else {
			prev = "上一页";
		}
		
		if(page < pageCount) {
			next = "<a href='"+request.getContextPath()+url+flag+"pageNum="+(page + 1)+"'>下一页</a>";
		} else {
			next = "下一页";
		}
